...
function getLastUpdated(lastUpdated: Date) {
  ...
  let _year = Math.round(timeElapsed / 31536000000);
  if (_year > 1) {
    return `${_year} years ago`;
  }
  let _month = Math.round(timeElapsed / 2592000000);
  if (_month > 1) {
    return `${_month} months ago`;
  }
  let _day = Math.round(timeElapsed / 86400000);
  if (_day > 1) {
    return `${_day} days ago`;
  }
  let _hour = Math.round(timeElapsed / 3600000);
  if (_hour > 1) {
    return `${_hour} hours ago`;
  }
  let _minute = Math.round(timeElapsed / 60000);
  if (_minute > 1) {
